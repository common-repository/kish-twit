...
/* If the oauth_token is old redirect to the connect page. */
	if (isset($_REQUEST['oauth_token']) && $_SESSION['oauth_token'] !== $_REQUEST['oauth_token']) {
		...
		header('Location:' .WP_PLUGIN_URL.'/kish-twit/oauth/clearsessions.php');
	}
	...
	$ktp_config_file=$ktproot.'/wp-content/plugins/kish-twit/functions.php';
	//file_exists($ktproot.'/wp-load.php') ? require_once($ktproot.'/wp-load.php') : require_once($ktproot.'/wp-config.php');
...
/* Request access tokens from twitter */
	...
	if (200 == $connection->http_code) {
	  ...
	  /* Save HTTP status for error dialog on connnect page.*/
		$goto = $_REQUEST['kgoto'];
	  header('Location: ' .WP_PLUGIN_URL.'/kish-twit/oauth/clearsessions.php?kgoto='.$goto);
	}
?>